...
public class ShardedObjectPool
{
    ...
    public ShardedObjectPool(int shardCount = 0)
    {
    ...
    // 获取指定类型的对象
    // <typeparam name="T">可池化类型（必须实现IResettable且有公开无参构造函数）</typeparam>
    // <returns>池化对象</returns>
    public T Rent<T>() where T : class, IResettable, new()
    {
        Type type = typeof(T);
        // 动态获取或创建该类型的子池（线程安全）
        var subPool = _typeToSubPools.GetOrAdd(type, t => new ShardedSubPool(t, _shardCount));
        // 从子池获取对象并强转为T
        return (T)subPool.Rent();
    }

    // 归还对象到池（自动根据对象类型找到对应子池）
    // <param name="obj">要归还的对象（必须是通过Rent获取的可池化对象）</param>
